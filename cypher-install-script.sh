...
# run script as sudo or root as some steps require elevated permissions
...
DEBIAN_FRONTEND=noninteractive apt -y install git build-essential cmake openssl libssl-dev gcc pkg-config libpcap-dev libz-dev
...
make -C ${path_cryptopant}/cryptopANT-1.3.1 install
