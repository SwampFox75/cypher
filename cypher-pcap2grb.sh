...
gbs_directory="/home/cypher-user/Desktop/cypher/gbs"
...
# script to take all but the latest pcap and convert it to gbs
...
# if there is more than one pcap then process them to gbs
# if there is only one pcap it is probably being written to by tcpdump
if [[ $pcap_count > 1 ]]; then
	for pcap in ${pcap_list[@]:1}; do
		echo "$(date '+%b %d %Y %H:%M:%S') $(hostname) running pcap2gbs on $pcap"
		pcap2grb -i $pcap -o $gbs_directory
